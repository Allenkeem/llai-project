...
def load_regions(unit: str) -> gpd.GeoDataFrame:
    """시도 경계를 권역으로 dissolve하고 LLAI를 병합."""
...
def make_static(unit: str, save: bool = True):
    plt.rcParams["font.family"] = "Malgun Gothic"
    plt.rcParams["axes.unicode_minus"] = False
    diss = load_regions(unit)
    ...
    ax.set_title(f"지역별 종합 법률 접근성 지수 LLAI ({unit}, 2024)")
    ax.axis("off")
    fig.tight_layout()
    if save:
        FIG.mkdir(parents=True, exist_ok=True)
        fig.savefig(FIG / f"fig7_map_{unit}.png", bbox_inches="tight", dpi=120)
    return fig


def make_interactive(unit: str, save: bool = True):
    import folium
    diss = load_regions(unit)
    ...
    if save:
        ...
        m.save(str(MAPS / f"llai_map_{unit}.html"))
    return m
